...
__global__ void convolution(int *input, int *output, int height, int width, int channels) {
	...
	int row = blockIdx.y * BLOCK_IDX + threadIdx.y;
	int col = blockIdx.x * BLOCK_IDX + threadIdx.x;
	int depth = blockIdx.z * blockDim.z + threadIdx.z;

    // Define shared memory for the operation (3 channel array?)
	__shared__ int sharedMem[BLOCK_IDX][BLOCK_IDX][channels];
	
    // Copy data to shared memory array
	sharedMem[threadIdx.y][threadIdx.x][threadIdx.z] = input[row * width + col + depth];
	__syncthreads();
